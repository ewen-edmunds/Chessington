...
namespace Chessington.GameEngine.Pieces
{
    public class Pawn : Piece
    {
        ...
        private List<Square> GetSquaresEnPassant(Board board, int moveDirection)
        {
            ...
            for (int colOffset =-1; colOffset < 2; colOffset+=2)
            {
                Square captureSquare = mySquare + new MoveOffset(0, colOffset);
                if (BoardQuery.IsWithinBounds(captureSquare) &&
                    BoardQuery.IsOppositeColourOnSquares(board, mySquare, captureSquare))
                {
                    if (board.previousBoard.GetPiece(captureSquare + new MoveOffset(moveDirection * 2, 0)) ==
                        board.GetPiece(captureSquare))
                    {
                        availableMoves.Add(captureSquare + new MoveOffset(moveDirection,0));
                    }
